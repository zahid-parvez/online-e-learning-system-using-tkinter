...
def admin_display_courses():
    admin_courses_window = Tk()
    admin_courses_window.title('  Courses Available  ')
    Label(admin_courses_window, text="   **  Courses Available  **   ", fg='indigo', font=("Wide Latin", 27),
          bg="grey").pack()
    admin_courses_window.geometry("700x500")
    option = IntVar()

    def display_specialized_courses():
        if option.get() == 1:
            admin_courses_window.destroy()
            cse.display()
        elif option.get() == 2:
            admin_courses_window.destroy()
            ece.display()
    ...
    R1_button = Radiobutton(admin_courses_window, text="IT/CSE/CS/CST courses", variable=option, value=1,
    ...
    R2_button = Radiobutton(admin_courses_window, text="ECE courses", variable=option, value=2,
                            font=('Arial Rounded MT Bold', 12), foreground="indigo", background="light green",
                            command=display_specialized_courses)
    R2_button.pack(anchor=CENTER, pady=4)

    R3_button = Radiobutton(admin_courses_window, text="EEE courses", variable=option, value=3,
                            font=('Arial Rounded MT Bold', 12), foreground="black", background="light green",
                            command=display_specialized_courses)
    R3_button.pack(anchor=CENTER, pady=4)

    R4_button = Radiobutton(admin_courses_window, text="Mechanical Engineering courses", variable=option, value=4,
                            font=('Arial Rounded MT Bold', 12), foreground="blue", background="light green",
                            command=display_specialized_courses)
    R4_button.pack(anchor=CENTER, pady=4)

    R5_button = Radiobutton(admin_courses_window, text="Civil Engineering courses", variable=option, value=5,
